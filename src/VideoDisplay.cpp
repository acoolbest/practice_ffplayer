...
void *video_refresh_timer(void *userdata)
{
	...
	if (video->stream_index >= 0)
	{
		...
		else
		{
			...
			double delay = (current_pts - video->frame_last_pts)/video->speed;
			...
			if (actual_delay <= 0.010) 
				actual_delay = 0.010; 
			//external_clock: %f, av_gettime() / 1000000.0, 
			//printf("diff[%f] current_pts[%f] ref_clock[%f], delay[%f]ms, actual_delay[%f], frame_timer[%f], external_clock[%f], count: %d\n", diff*1000, current_pts, ref_clock, delay*1000, actual_delay*1000, video->frame_timer, av_gettime() / 1000000.0, count++);
